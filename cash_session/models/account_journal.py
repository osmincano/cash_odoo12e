...
class AccountJournal(models.Model):
    ...
    journal_user = fields.Boolean('Use in Cash Session',
                                  help="Check this box if this journal define a payment method that can be used in a cash.")
    amount_authorized_diff = fields.Float('Amount Authorized Difference',
                                          ...
                                          "closing a session, for non-CASH managers. If this maximum is reached, the user will have an error message at "
                                          "the closing of his session saying that he needs to contact his manager.")

    @api.model
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        session_id = self.env.context.get('cash_session_id', False)
        if session_id:
            session = self.env['cash.session'].browse(session_id)
            if session:
